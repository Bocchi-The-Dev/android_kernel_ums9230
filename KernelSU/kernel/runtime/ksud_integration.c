...
#ifdef CONFIG_KSU_MANUAL_HOOK
    // In manual-hook mode there is no input kprobe; the manually-patched
    // input_event()/input_handle_event() consult ksu_input_hook directly.
    ksu_input_hook = false;
    pr_info("manual_hook: input hook stopped!\n");
#else
...
#ifdef CONFIG_KSU_MANUAL_HOOK
    // Safe-mode detection is served by the manually-patched input
    // functions; registering the kprobe as well would count every
    // volume key twice.
    pr_info("ksud: manual hook enabled, skipping input_event kprobe\n");
#else
